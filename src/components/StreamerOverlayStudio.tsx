...
import { GameStateEventType, OverlayLayoutType, StreamerGameStateEvent, ActiveBillboardSlot } from '../types';
import {
  Tv,
  Radio,
  Zap,
...
export const StreamerOverlayStudio: React.FC<StreamerOverlayStudioProps> = ({
  ...
  // Twitch, YouTube, Kick Live Connect State
  const [syncPlatform, setSyncPlatform] = useState<'twitch' | 'youtube' | 'kick'>('twitch');
  const [channelNameInput, setChannelNameInput] = useState<string>('');
  const [liveSyncData, setLiveSyncData] = useState<any>({
    isLive: true,
    concurrentViewers: 3450,
    effectiveCpm: 8.50
  });
  const [isConnectingPlatform, setIsConnectingPlatform] = useState<boolean>(false);

  // Custom Audio Chime State
  const [selectedChime, setSelectedChime] = useState<'arcade' | 'cyber' | 'kaching' | 'bell'>('kaching');
  ...
  const handleConnectPlatform = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/creators/connect-platform', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          handle: cleanHandle,
      ...
      if (data.success && data.creator) {
        setLiveSyncData(data.creator);
        soundEffects.playVictoryFanfare();
      }
    } catch (e) {
      console.warn('Connect platform error:', e);
    } finally {
  ...
  const handleTestChime = () => {
    if (selectedChime === 'kaching') soundEffects.playKaChing();
    else if (selectedChime === 'arcade') soundEffects.playCoinDrop();
    else if (selectedChime === 'cyber') soundEffects.playVictoryFanfare();
    else soundEffects.playTone(880, 0.2);
  };
  ...
  const fetchStats = async () => {
    try {
      ...
      // Fetch live platform sync data
      const syncRes = await fetch(`/api/creators/live-sync/${cleanHandle}`);
      if (syncRes.ok) {
        const syncData = await syncRes.json();
        if (syncData.creator) setLiveSyncData(syncData.creator);
      }
    } catch {
      // Safe fallback
    }
      ...
      {activeTab === 'url_builder' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5">
            ...
            {/* Live Streaming Platform Connect Hub (Twitch, YouTube, Kick) */}
            <div className="p-4 bg-slate-950/90 border border-purple-500/40 rounded-2xl space-y-3 shadow-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Tv className="w-4 h-4 text-purple-400" />
                  <span className="text-xs font-black text-white uppercase">Live Creator Sync (Twitch • YouTube • Kick)</span>
                </div>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 font-bold flex items-center gap-1">
                  ...
                  <span>{liveSyncData?.concurrentViewers?.toLocaleString() || '3,450'} Viewers Live</span>
                </span>
              </div>

              <form onSubmit={handleConnectPlatform} className="flex gap-2 text-xs">
                <select
                  value={syncPlatform}
                  onChange={(e) => setSyncPlatform(e.target.value as any)}
                  className="bg-slate-900 border border-slate-700 text-purple-300 font-bold px-3 py-2 rounded-xl focus:outline-none"
                >
                ...
                <input
                  type="text"
                  placeholder="Channel username (e.g. KaiCenat)..."
                  value={channelNameInput}
                  onChange={(e) => setChannelNameInput(e.target.value)}
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-purple-400 font-mono"
                />

                <button
                  type="submit"
                  disabled={isConnectingPlatform}
                  className="px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-bold rounded-xl transition shadow flex items-center gap-1 cursor-pointer disabled:opacity-50"
                >
                  <span>{isConnectingPlatform ? 'Syncing...' : 'Sync Viewers'}</span>
                </button>
              </form>

              <div className="grid grid-cols-2 gap-2 text-[11px] font-mono pt-1">
                <div className="bg-slate-900/80 p-2 rounded-xl border border-slate-800">
                  <span className="text-slate-400 text-[10px] block font-sans">Effective Audience CPM</span>
                  <span className="text-amber-400 font-bold text-xs">${liveSyncData?.effectiveCpm?.toFixed(2) || '8.50'} / 1k Impr</span>
                </div>
                <div className="bg-slate-900/80 p-2 rounded-xl border border-slate-800">
                  <span className="text-slate-400 text-[10px] block font-sans">Active Payout Share</span>
                  <span className="text-emerald-400 font-bold text-xs">70% Direct to Wallet</span>
                </div>
              </div>
            </div>

            {/* Custom Audio Chime Stinger Selector */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-300 font-mono flex items-center gap-1.5">
                  <Volume2 className="w-3.5 h-3.5 text-cyan-400" />
                  <span>Ad Takeover Audio Chime:</span>
                </label>
                <button
                  type="button"
                  onClick={handleTestChime}
                  className="text-[10px] text-cyan-400 hover:text-cyan-300 flex items-center gap-1 font-mono font-bold cursor-pointer"
                >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono font-bold">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setSelectedChime(chime.id as any);
                      if (chime.id === 'kaching') soundEffects.playKaChing();
                      else if (chime.id === 'arcade') soundEffects.playCoinDrop();
                      else if (chime.id === 'cyber') soundEffects.playVictoryFanfare();
                      else soundEffects.playTone(880, 0.2);
                    }}
                    className={`p-2 rounded-xl border text-center transition-all cursor-pointer ${
                      selectedChime === chime.id
                        ? 'bg-cyan-950/80 border-cyan-500 text-cyan-300 shadow ring-1 ring-cyan-500'
                        : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-white'
                    }`}
                  >
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-3">
              ...
              {/* Live Interactive Preview Canvas (Connected to Real 15s Slot Data & Scannable QR Barcode) */}
              {(() => {
                const liveAd = slotData?.winningAd || {
                  id: 'fallback_live',
                  title: previewAd.title,
                  imageUrl: previewAd.imageUrl,
                  advertiserName: previewAd.advertiser,
                  bidAmountCents: Math.round(parseFloat(previewAd.bidDollars) * 100),
                  mediaType: 'image' as const,
                  ctaUrl: 'https://www.livebillboards.lol/watcher'
                };
                const isVideo = liveAd.mediaType === 'video' || liveAd.imageUrl?.startsWith('data:video/') || liveAd.imageUrl?.toLowerCase().includes('.mp4');
                const qrDestination = liveAd.ctaUrl || liveAd.landingPageUrl || 'https://www.livebillboards.lol/watcher';
                const secondsLeft = slotData?.remainingSeconds ?? 15;
                const bidFormatted = ((liveAd.bidAmountCents || 100) / 100).toFixed(2);
                ...
                return (
                  <div className={`relative aspect-video w-full rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 shadow-2xl flex items-center justify-center select-none ${
                    ...
                    {/* Live Video or Image Creative Display */}
                    {isVideo ? (
                      <video
                        key={liveAd.imageUrl}
                        src={liveAd.imageUrl}
                    ...
                    <div className="absolute top-2.5 inset-x-2.5 flex items-center justify-between pointer-events-none z-10">
                      ...
                      <div className="px-2.5 py-1 bg-rose-950/90 border border-rose-500/60 backdrop-blur-md text-rose-300 font-mono font-black text-[11px] rounded-xl shadow-lg flex items-center gap-1.5">
                        <Clock className="w-3.5 h-3.5 animate-spin" style={{ animationDuration: '8s' }} />
                        <span>⏱️ {secondsLeft}s</span>
                      </div>
                    </div>

                    {/* Dynamic Scannable Customer QR Barcode (If Enabled) */}
                    {showQr && (
                      <div className="absolute bottom-11 right-2.5 bg-slate-950/95 border-2 border-amber-400 p-2 rounded-2xl shadow-2xl flex items-center gap-2 backdrop-blur-md z-10">
                        <div className="p-1 bg-white rounded-xl shadow">
                          <img
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&margin=2&data=${encodeURIComponent(qrDestination)}`}
                            alt="Scan To Claim QR"
                            className="w-12 h-12 object-contain"
                          />
                        </div>
                        <div className="text-left font-mono">
                          <div className="text-[9px] font-black text-amber-300 uppercase leading-none">
                            ⚡ SCAN ME
                          </div>
                          <div className="text-[8px] text-slate-300 font-semibold mt-0.5 max-w-[90px] truncate">
                            {qrDestination.replace(/^https?:\/\//, '')}
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {showTicker && (
                      <div className="absolute bottom-0 inset-x-0 bg-slate-950/90 border-t border-slate-800 px-3 py-2 flex items-center justify-between text-[11px] backdrop-blur-md font-mono pointer-events-none z-10">
                        <div className="text-white font-bold truncate max-w-[200px]">
                          {liveAd.title}
                        </div>
                        <div className="text-cyan-300 shrink-0 flex items-center gap-1.5 font-bold">
                          <span>Sponsor: {liveAd.advertiserName || 'Live Sponsor'}</span>
                          <span className="text-amber-400 font-mono">(${bidFormatted})</span>
                        </div>
                      </div>
                    )}
                  </div>
